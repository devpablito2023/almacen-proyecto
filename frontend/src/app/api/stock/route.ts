...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validar campos requeridos para ajuste de stock
    const requiredFields = ['producto_id', 'tipo_ajuste', 'cantidad_ajuste', 'motivo'];
    ...
    if (!['positivo', 'negativo'].includes(body.tipo_ajuste)) {
      return NextResponse.json(
        { success: false, message: 'tipo_ajuste debe ser "positivo" o "negativo"' },
        { status: 400 }
      );
    }
    ...
    if (typeof body.cantidad_ajuste !== 'number' || body.cantidad_ajuste <= 0) {
      return NextResponse.json(
        { success: false, message: 'cantidad_ajuste debe ser un número positivo' },
        { status: 400 }
      );
    }

    const response = await fetch(`${BACKEND_URL}/stock/ajuste`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
